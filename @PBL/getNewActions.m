...
% Draws new actions to query using obj.drawSamples
...
if obj.settings.acq_type == 1 && obj.settings.useSubset == 1
...
[samples, globalinds, visitedinds, rewards] = obj.drawSamples(iteration);
obj.iteration(iteration).samples.actions = samples;
obj.iteration(iteration).samples.globalInds = globalinds;
obj.iteration(iteration).samples.visitedInds = visitedinds;
obj.iteration(iteration).samples.rewards = rewards;
...
if any(obj.settings.feedback_type == 1)
    if length(compared_actions_visited) > 1
        ...
        elseif length(compared_actions_visited) > 2
            [comparisonInds, ~] = utils.rankingToPreferences(1:length(compared_actions_global));
