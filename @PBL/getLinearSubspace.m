...
%%% Coordinate Aligned:
if obj.settings.isCoordinateAligned
    ...
    while (randAction == current_best)
        randAction = obj.getRandAction(1);
%         a0_ind = randi(obj.settings.num_actions,1,1);
%         randAction = obj.settings.points_to_sample(a0_ind,:); 
    end
...
%     
    ...
    n = 1;
    uniformdisc = 1/100;
    new_point = current_best + n.*uniformdisc.*direction;
...
%     [~,nearestGlobalInd] = min(vecnorm(point - obj.settings.points_to_sample,2,2));    
%     nearestPoint = obj.settings.points_to_sample(nearestGlobalInd,:);
end
