...
if obj.settings.useSyntheticObjective
    true_utility = obj.settings.true_objectives(obj.iteration(iteration).best.globalInd);
    obj.iteration(iteration).best.true_utility = true_utility;
    obj.iteration(iteration).best.optimal_error = obj.settings.true_bestObjective - true_utility;
    
    % get fit error
    post_vals = obj.post_model(iteration).mean;
    true_vals = obj.settings.true_objectives(obj.post_model(iteration).action_globalInds);
