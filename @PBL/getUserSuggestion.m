function feedback = getUserSuggestion(obj, iteration)
...
feedback = struct('c_x_subset',[],'c_x_full',[],'c_y', []);
...
if any(obj.settings.feedback_type == 2)
    ...
    num_dims = length(obj.settings.parameters);
    for n = 1:num_actions
        
        % get the action that is being compared to:
        current_action = obj.iteration(iteration).samples.actions(n,:);
        current_globalInd = obj.iteration(iteration).samples.globalInds(n);
        current_visitedInd = obj.iteration(iteration).samples.visitedInds(n);
        ...
        if strcmpi(isFeedback,'y')
            ...
            dim = input(sprintf(['Enter dimensions to give feedback ', ...
                ...
                num_dims, obj.settings.parameters(:).name));
            ...
            feedback.c_x_subset = zeros(length(dim),2);
            feedback.c_x_full = zeros(length(dim),2);
            feedback.c_y = 2*ones(length(dim),1);
            ...
            for d = 1:length(dim)
                ...
                if direction == 1
                    ...
                    delta(d) = -obj.settings.parameters(dim(d)).discretization;
                ...
                suggested_action = boundAction(obj,suggested_action);
                
                if ~isequal(suggested_action,current_action)
                    
                    % find globalInd closest to suggested action
                    suggested_globalInd = obj.getGlobalInd(suggested_action);
                    
                    % get visitedInd for suggested action
%                     suggested_visitedInd = obj.getVisitedInd(suggested_action);
%                     
                    
                    % append coactive point to output
                    feedback.c_x_subset(d,:) = [current_visitedInd, 0];
                    feedback.c_x_full(d,:) = [current_globalInd, suggested_globalInd];
...
zero_rows = all(feedback.c_x_subset == 0,2);
feedback.c_x_subset(zero_rows,:) = [];
feedback.c_x_full(zero_rows,:) = [];
feedback.c_y(zero_rows,:) = [];
