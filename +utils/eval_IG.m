function [newSampleGlobalInd,newSamples] = eval_IG(obj, R, select_idx,buffer_action_idx, iteration)
...
if ~isfield(obj.settings,'num_ord_cat') || ~any(obj.settings.feedback_type == 3)
    num_ord_cat = 0;
else
    num_ord_cat = obj.settings.num_ord_cat;
end
ord_cat = num_ord_cat;
pref_choice = [1,2]; 
...
pref_comb = utils.permn(pref_choice,num_pref);

num_ord = num_samples; %new ordinal labels
ord_comb = utils.permn(ord_choice,num_ord);
...
for i = 1:size(all_action_comb,1)
   ...
   if any(obj.settings.feedback_type == 3) && any(obj.settings.feedback_type == 1)
        ...
        for k = 1:size(pref_ord_comb,1)
            ...
            pref_prod = utils.eval_pref_prod(s,y_pref,obj);
            ord_prod = utils.eval_ord_prod(o,y,obj);
...
newSamples = obj.settings.points_to_sample(newSampleGlobalInd,:);
end   
